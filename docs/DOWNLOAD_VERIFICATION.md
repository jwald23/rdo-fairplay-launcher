...
The CLI may ask you to authenticate with GitHub. Verification checks the file digest and signed build record against this repository and its release workflow. Stop if verification fails; do not run the file until the failure is understood.
...
Expand **Installer SHA256** on the same release page as your installer. Run:
...
Compare the entire hash with the installer hash in the release notes; letter case does not matter. A matching checksum detects changed or damaged downloads. A checksum alone does not establish trust in its publisher.

## VirusTotal reports and release evidence

Release notes contain a short checks summary and an exact-installer VirusTotal link. Detailed hash and dependency records are available in the linked GitHub Actions run under its **verification** artifact (retained for 90 days; downloading may require GitHub sign-in). Historical published scan records are preserved in [releases](releases/). These optional records are not needed to install FairPlay.
